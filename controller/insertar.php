...
if(strlen($identificacion) > 0 && strlen($nombre) > 0 && strlen($apellido) > 0 && strlen($email) > 0 && strlen($cargo) > 0  && strlen($pass) > 0 && strlen($estado) > 0){
    $passmd = md5($pass);
    define ('LIMITE',2000);
        ...
        if ($_FILES["foto"]["error"]>0) {//ese file depende del name del input del registrar//
			...
			echo "<script>location.href='../views/registrar-usuario-admin.php'</script>";
		}else{
            if (in_array($_FILES['foto']['type'], $FORMATO) && $_FILES['foto']['size'] <= LIMITE *2000){
                ...
                if (!file_exists($rutaimg)){
                    ...
                    if ($resultado){
                        ...
                        $result = $consulta->insertUsers($identificacion, $nombre, $apellido, $email, $cargo, $passmd, $estado, $rutaimg);
                    }else{
